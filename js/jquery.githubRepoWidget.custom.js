(function($){
	...
	window.ghw = function () {
		...
		const ghwidgets = document.querySelectorAll('.github-widget');

		ghwidgets.forEach((e, i) => {
		...
		if (vendors){
			...
			for (let vendor in vendors){
				fetch({
					...
					Array.from(results.data).forEach((value, index) => {
						const repo = value;
						const vendorUrl = `https://github.com/${vendor}`,
							repoUrl = `https://github.com/${vendor}/${repo.name}`;

						const innerWidgetHeader$ = document.createElement('h3');
						innerWidgetHeader$.innerHTML = `
						<a class="owner" target="_blank" href="{vendorUrl}" title="${vendorUrl}">${vendor}</a>/
						<a class="repo" target="_blank" href="${repoUrl}" title="${repoUrl}">${repo.name}</a>
						`;

						const innerWidgetStats$ = document.createElement('div');
						innerWidgetStats$.className = 'github-stats';
						innerWidgetStats$.innerHTML = `
						<a class="watchers" target="_blank" href="${repoUrl}/watchers" title="See watchers">?</a>
						<a class="forks" target="_blank" href="${repoUrl}/network/members" title="See forkers">?</a>
						`;

						const innerWidgetContent$ = document.createElement('div');
						innerWidgetContent$.className = 'github-box-content';
						innerWidgetContent$.innerHTML = `
						<p class="description"><span></span> &mdash; <a href="${repoUrl}#readme">Read More</a></p>
						`;

						const innerWidgetDownload$ = document.createElement('div');
						innerWidgetDownload$.className = 'github-box-download';
						innerWidgetDownload$.innerHTML = `
						<div class="updated"></div>
						<a class="download" target="_blank" href="${repoUrl}/zipball/master" title="Get an archive of this repository">Download as zip</a>
						`;

						const innerWidget$ = document.createElement('div');
						innerWidget$.className = 'github-box-title';
						innerWidget$.appendChild(innerWidgetHeader$);
						innerWidget$.appendChild(innerWidgetStats$);
						innerWidget$.appendChild(innerWidgetContent$);
						innerWidget$.appendChild(innerWidgetDownload$);

						const widget$ = document.createElement('div');
						widget$.className = 'github-box repo';
						widget$.appendChild(innerWidget$);
						...
						if(repo.homepage != null) {
							...
							link$.innerText(repo.homepage);
							widget$.find('.link').append(link$);
						}
